...
print("⏳ Фикшу чтение полей структур внутри выражений...")
...
with open("main.go", "w", encoding="utf-8") as f:
    f.write(go_code.strip() + "\n")

print("✅ Файл main.go исправлен под чтение полей в математике.")
print("🛠️ Перекомпилирую...")

result = subprocess.run(["go", "build", "-o", "codex.exe", "main.go"], capture_output=True, text=True)

if result.returncode == 0:
    print("🔥 ГОТОВО! Математика с полями структур полностью поддерживается!")
else:
    print("❌ Ошибка сборки:")
    print(result.stderr)